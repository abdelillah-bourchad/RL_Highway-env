...
models_dir = f"models_gamma{int(100*gamma)}_{policy}_col{collision_reward}_arr{arrived_reward}_spd{high_speed_reward}/PPO"
...
model = PPO(policy, env, verbose=2, tensorboard_log=logdir
            , gamma=gamma
...
if TIMESTEPS > n_steps:
    ...
    for i in range(n_iterations):
        model.learn(total_timesteps=n_steps, reset_num_timesteps=False, tb_log_name=tb_log_name)
        model.save(f"{models_dir}/{model.num_timesteps}")
        print(f"Iteration {i+1}/{n_iterations} done")